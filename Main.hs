...
data Route a where
  ...
  -- | Redirect to an URL
  Route_Redirect :: FilePath -> Route Text
...
generateSite = do
  ...
  void $ Z.generateSite (writeHtmlRoute . Route_Zettel) ["*.md"]
  forM_ oldLinkRedirects $ \(oldPath, newUrlRel) ->
    writeHtmlRoute (Route_Redirect oldPath) newUrlRel
  where
    oldLinkRedirects =
      [ ("tidbits/dhall-toml-free-monad.html", "/2002201.html"),
        ("haskell-nix.html", "/1948201.html"),
        ("lasik.html", "/1911401.html"),
        ("conflicts.html", "/1930301.html"),
        ("calisthenics.html", "/1915701.html"),
        ("carnivore-diet.html", "/1918401.html"),
        ("sous-vide.html", "/1919301.html")
      ]
  ...
  head_ @(Html ()) $ do
    case route of
      ...
      Route_Zettel zr -> do
        ...
        style_ [type_ "text/css"] $ C.render $ style site
