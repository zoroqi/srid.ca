...
This is possible using [nixos-generators](https://github.com/nix-community/nixos-generators). First, create a LXD image for NixOS based on a `configuration.nix` (see [mine](https://github.com/srid/nix-config/blob/master/machine/bare.nix)):
...
If the configuration.nix also declares an user account, you can instead directly log in as that user:
