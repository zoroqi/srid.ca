...
The official image server for LXD does not support NixOS. However, we can build our own using [nixos-generators](https://github.com/nix-community/nixos-generators). Let's wrap the commands in a script (call it [`buildLxcImage.sh`](https://github.com/srid/nix-config/blob/master/nixos-configuration/buildLxcImage.sh)):
...
You will need a `configuration.nix` (see [mine](https://github.com/srid/nix-config/blob/master/nixos-configuration/bare.nix)) to build a NixOS image, using this script:
